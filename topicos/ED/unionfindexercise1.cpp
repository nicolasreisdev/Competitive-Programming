...
const int MAX = 2 * 1e5 + 1; // o valor é dobrado pois faremos 2 conjuntos, 1 de amigos e 1 de inimigos
...
int friends(int x)
{ // retorna o amigo de x
    return x;
}

int enimies(int x)
{ // retorna o inimigo de x
    return MAX / 2 + x;
}
...
{ // verifica se x e y são do mesmo grupo
    return (find(x) == find(y));
}

void merge(int x, int y)
{ // junta os grupos de x e y
    int a = find(x);
    int b = find(y);
    if (tam[a] < tam[b])
    {
...
int main()
{
    ...
    while (cin >> c >> x >> y && c || x || y)
    {
        if (c == 1)
        { // se são amigos
            if (same(friends(x), enimies(y)))
            { // se os amigos de x são inimigos de y
                cout << -1 << endl;
            }
            else
            {
        ...
        { // se são inimigos
            if (same(friends(x), friends(y)))
            { // se os amigos de x são amigos de y
                cout << -1 << endl;
            }
            else
            {
