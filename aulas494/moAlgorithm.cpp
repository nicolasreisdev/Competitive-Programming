...
{
public:
    JanelaMo(vector<int> &v_) : v(v_)
    { // tamanho do array
        soma = 0;
        n = 0;
    }
    double valorConsulta()
    { // adaptar: retorna o valor da consulta da janela atual
        return ((double)soma) / n;
    }
    void add(int idx)
    { // adaptar:adiciona o elemento da posicao idx a janela
        soma += v[idx];
        n++;
    }
    void remove(int idx)
    { // adaptar:remove o elemento da posicao idx da janela
        soma -= v[idx];
        n--;
    }
